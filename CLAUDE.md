...
- **Multi-provider routing** — `agent/core.py` `Agent` is provider-aware: a single agent can run Claude, Kimi and GPT models. `agent/providers.py` resolves a model ID to its provider by prefix (`kimi-*`→kimi, `gpt-*`/`codex-*`/`o3*`→openai, **everything else**→claude_code). Kimi speaks the Anthropic wire protocol via its Anthropic-compatible endpoint (base URL `https://api.kimi.com/coding/`) and so uses `_run_anthropic`; OpenAI uses the ChatGPT-backend Responses API. Provider is resolved per `run()` call, so workers build one agent and use any model.
- **There is no metered Anthropic provider** — it was removed because it required pinning dated model IDs (`claude-sonnet-4-6`) that had to be hand-edited each release, while the Claude Code CLI accepts `haiku`/`sonnet`/`opus` and resolves each to the current model itself. Consequences worth knowing: the `resolve_provider` fallback is `claude_code`, so a leftover `claude-sonnet-4-6` setting resolves to the subscription rather than to a provider that no longer exists; `providers.claude_code_model()` collapses a dated Claude ID onto its alias so those settings keep working; and `ANTHROPIC_PROTOCOL` is now `(KIMI,)` — the grouping survives because `_run_anthropic` is written against the wire protocol, not a vendor.
...
- **Model IDs** — Claude models are named by **alias, not version**: `claude-code/haiku`, `claude-code/sonnet`, `claude-code/opus`. Never pin a dated Claude ID — the CLI resolves the alias to the current model, which is the whole reason the metered provider was dropped. Others: `kimi-for-coding`, and the GPT-5.6 tiers `gpt-5.6-sol` (flagship) / `gpt-5.6-terra` (workhorse) / `gpt-5.6-luna` (fast, cheap). `gpt-5.5` still works but is previous-generation; `codex-5.3`/`gpt-5.3-codex` are deprecated for ChatGPT sign-in and `gpt-5.4`/`gpt-5.4-mini` retired from Codex on 2026-08-31.
...
- **Memory extraction** — Runs in background after each chat. Prefers the Claude subscription (`_extract_via_claude_code`, no tools registered), then Kimi's Anthropic-compatible endpoint; OpenAI is skipped because it speaks a different API and extraction is best-effort. The subscription path exists so removing the metered Anthropic provider did not leave extraction silently dependent on Kimi being configured. Extracts structural facts, not transient state. Note the asymmetry: memories are **written only by chat** (`api/chat.py` is the sole caller of `extract_memories`) but **read by every agent** through `get_prompt()`. Facts learned during an alert investigation, a PR review, or outside this agent entirely have no automatic path in — that is what `POST /api/memories` is for.
...
- **Cleared alerts are dropped before triage** — Alertmanager sends both the firing and the clearing notification through ntfy, distinguished only by a `[RESOLVED]` marker in the title. `is_resolved()` skips the clearing one: there is nothing to diagnose once the condition is gone, and triaging it anyway cost a full agent run to restate what the title says (one such run spent 18k tokens and four tool calls before answering "the alert title shows [RESOLVED]"). `alert_identity()` also strips the marker from the cooldown key, so a fire/clear pair collapses onto one key — with it left in they were two different alerts, and every alert cost two triage runs minutes apart.
...
Three providers can be configured simultaneously (any subset). The model assigned to each task picks the provider. All of them bill a subscription or plan; nothing here is metered per token, which is why `costs.py` prices everything at $0 and `_DEFAULT_PRICING` is $0 rather than a Sonnet-shaped guess.
...
Model keys (used in `models.py` defaults and DB `model_*` settings): `pr_review`, `alert_triage`, `alert_fix`, `code_fix`, `deep_review`, `chat`. Every default is a `claude-code/*` alias, so a fresh deploy needs only a `claude setup-token` value to work.
